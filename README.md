...
DeepSeek 官方兼容端点可直接用；第三方网关未开 CORS 时，浏览器版用仓库根 `.env.local` 配
`LLM_PROXY_TARGET` 走 dev 代理（baseURL 填 `/llm/v1`），桌面版设置页一键改走壳内本地代理。
多套端点（URL/模型/Key）可在设置页存成「预设」，随时一键切换。
...
测试基线：逻辑测试 931 全绿（typecheck 零错误 / 生产构建通过）。桌面版（v7 Tauri 壳：本地代理根治 CORS、全库导出导入迁移桥、GitHub Releases 自动更新）已入仓，NSIS 安装包由 CI（release-desktop.yml，推 tag 触发）出包。
